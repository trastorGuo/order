  ...
  <div id="app" style="height: 1000px;width: 100%;">
    <v-expansion-panels multiple style="padding: 15px !important;" v-model="panel">
      <v-expansion-panel style="margin-bottom: 10px !important;" v-for="(date,index) in allData" :key="index">
        <v-expansion-panel-header style="background: #28b89c;">{{ date.DATE }}   <v-spacer></v-spacer>日营业额：${{date.DATE_PRICE}}</v-expansion-panel-header>
...
export default {
  ...
  mounted() {
    let self = this;
    self.$http("get", "/api/Product/GetOrders").then((response) => {
      if (!response.success) {
      ...
      response.data.forEach((element) => {
        ...
        element.ORDER.forEach((order) => {
          ...
          DATE_PRICE +=ALL_PRICE;
           order.ORDER_DATE = self.$moment(order.ORDER_DATE).format("YYYY-MM-DD HH:MM:SS");
        });
